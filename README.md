...
- **机场识别**：自动读取 Clash Verge Rev 订阅（profiles.yaml），顶部显示当前激活的机场和订阅数；在 Clash 切换订阅后点"重新检测"即可刷新
...
3. 保持 Clash Verge Rev 正常运行（自动通过命名管道对接，无需配置）
4. 点击【开始测速】即可

> 当前版本以 **Clash Verge Rev（Windows）** 为基准对接（自动识别 mixed-port 和订阅）。
...
- 通过 **Windows 命名管道** `\\.\pipe\verge-mihomo` 对接 Clash Verge Rev 内核（失败回退 TCP 9097/9090），零配置
